...
>>> import qualified Numeric.Dimension.Do as D 
>>> import Numeric.Dimension.Units 
>>> :{
>>> D.do 
>>>    x <- inv $ 3 `dim` "meter"
...
-}
module Numeric.Dimension.Do (fmap,pure,return,(<*>),(>>=),(>>),join,mult,inv) where
import Prelude hiding (pure,return,(<*>),(>>=),(>>))
import Numeric.Dimension.Data (Dimension(MkDimension))
import Numeric.Dimension.Units (type (!*), Invert,type (!^))
